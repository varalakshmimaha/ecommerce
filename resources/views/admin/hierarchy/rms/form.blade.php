...
<div class="max-w-4xl">
    ...
    <form method="POST" enctype="multipart/form-data"
          action="{{ $rm->exists ? route('admin.rms.update', $rm) : route('admin.rms.store') }}"
          class="space-y-6">
        @csrf
        ...
        <div class="bg-white rounded-xl shadow-sm border border-ui-border p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email', $rm->email) }}" required class="w-full px-3 py-2 border border-ui-border rounded-lg">
                </div>
                <div>
                ...
                <div class="md:col-span-2">
                    ...
                    @if(auth()->user()->role === 'manager' && !auth()->user()->is_admin)
                        {{-- Manager creating RM: locked to their own account --}}
                        <input type="hidden" name="parent_id" value="{{ auth()->id() }}">
                        <input type="text" value="{{ auth()->user()->name }}" disabled class="w-full px-3 py-2 border border-ui-border rounded-lg bg-gray-50 text-text-muted cursor-not-allowed">
                    @else
                        <select name="parent_id" required class="w-full px-3 py-2 border border-ui-border rounded-lg">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-ui-border p-6">
            ...
            @php $profile = $rm->affiliateProfile; @endphp
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-ui-border p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="md:col-span-2">
                    <label class="block text-sm font-medium text-text-heading mb-1">KYC Document {{ $rm->exists ? '(leave blank to keep existing)' : '*' }} <span class="text-text-muted font-normal">(PAN / Aadhaar copy — JPG, PNG, PDF, max 2MB)</span></label>
                    ...
                    @endif
                    <input type="file" name="kyc_doc" {{ $rm->exists ? '' : 'required' }} accept=".jpg,.jpeg,.png,.pdf" class="w-full px-3 py-2 border border-ui-border rounded-lg text-sm">
                </div>
            </div>
        </div>

        <div class="flex items-center gap-3">
            <button class="bg-gradient-to-r from-brand-gold via-brand-amber to-brand-crimson text-white px-6 py-2.5 rounded-lg font-semibold">{{ $rm->exists ? 'Update RM' : 'Create RM' }}</button>
