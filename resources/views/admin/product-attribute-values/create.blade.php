...
<div>
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-900">Create Product Attribute Value</h2>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form action="{{ route('admin.product-attribute-values.store') }}" method="POST">
            @csrf
                ...
                <div>
                    ...
                    <select id="product_attribute_id" 
                        ...
                        @foreach($attributes as $attribute)
                            <option value="{{ $attribute->id }}" {{ old('product_attribute_id') == $attribute->id ? 'selected' : '' }}>
                                {{ $attribute->name }}
                            </option>
                        @endforeach
                    </select>
