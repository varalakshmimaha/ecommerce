...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
    ...
    <div class="lg:col-span-2">
        <div class="admin-card">
            ...
            <div class="space-y-4 max-h-96 overflow-y-auto">
                ...
                <div id="brand-{{ $brand->id }}" class="border rounded-lg p-4"
                ...
                >
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4 flex-1">
                            @if($brand->icon)
                            <img src="{{ asset('storage/' . $brand->icon) }}" alt="{{ $brand->name }}" class="w-12 h-12 object-contain rounded-lg bg-gray-100 p-1">
                            @else
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-xs text-gray-500">{{ $brand->products_count ?? 0 }} products</p>
                            </div>
                        </div>
                        <div class="flex space-x-2">
                            <button type="button" onclick="editBrand({{ $brand->id }})" class="text-brand-gold text-sm">Edit</button>
    ...
    <div class="space-y-6">
        ...
        <div class="admin-card">
            ...
            <form action="{{ route('admin.brands.store') }}" method="POST" enctype="multipart/form-data" class="space-y-3">
                @csrf
                <div>
                ...
                <button type="submit" class="bg-gradient-to-r from-brand-gold via-brand-amber to-brand-crimson text-white px-6 py-3 rounded-lg font-semibold hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-300 w-full text-sm py-2">Add Brand</button>
            </form>
        </div>
    </div>
</div>
