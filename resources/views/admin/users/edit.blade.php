...
<div class="max-w-3xl">
    <div class="mb-6">
        <a href="{{ route('admin.users.index') }}" class="text-sm text-text-muted hover:text-text-heading inline-flex items-center gap-1 mb-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            Users
        </a>
        <h1 class="text-3xl font-bold text-text-heading">Edit User</h1>
        <p class="text-text-muted mt-1 text-sm">Update account details, role, and admin permissions.</p>
    </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-ui-border p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="mobile" value="{{ old('mobile', $user->mobile) }}" required maxlength="15"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-text-heading mb-1">Password <span class="text-text-muted font-normal">(leave blank to keep)</span></label>
            ...
            <div>
                ...
                <input type="text" name="role" value="{{ old('role', $user->role) }}" required
                    placeholder="e.g. admin, manager, supervisor, support…"
                ...
                <p class="text-xs text-text-muted mt-1">Use <strong>admin</strong> to grant full access.</p>
            </div>

            {{-- Permissions --}}
            <div>
                <label class="block text-sm font-medium text-text-heading mb-3">Admin Permissions</label>
                <p class="text-xs text-text-muted mb-3">Ignored if role is <strong>admin</strong> (they get full access).</p>
                @php
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                    @foreach($permList as $key => $meta)
                        ...
                        <label class="perm-card flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all select-none"
                               style="border-color: {{ $checked ? $meta['color'] : '#e5e7eb' }}; background: {{ $checked ? 'rgba(0,0,0,0.04)' : '#fff' }};"
