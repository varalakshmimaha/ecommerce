    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-lg shadow">
        ...
        <form id="filter-form" class="flex flex-col md:flex-row gap-2">
            <div class="flex gap-2">
                <input type="date" id="start_date" name="start_date" value="{{ $startDate }}" class="input-field px-3 py-2 border border-gray-300 rounded">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white p-6 rounded-lg shadow">
            ...
            <div class="space-y-2">
                ...
                <div class="flex justify-between items-center py-2 text-lg font-bold text-primary-600 bg-primary-50 px-3 py-3 rounded mt-4">
...
function loadReportData() {
    ...
    fetch(`{{ url('admin/reports/overall-sales') }}?start_date=${startDate}&end_date=${endDate}&json=1`)
        .then(res => res.json())
        .then(data => {
...
function downloadReport() {
    const startDate = document.getElementById('start_date').value;
    const endDate = document.getElementById('end_date').value;
    window.location.href = `{{ url('admin/reports/overall-sales') }}?start_date=${startDate}&end_date=${endDate}&download=1`;
}
