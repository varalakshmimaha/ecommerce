                        ...
                        @if($query->admin_response)
                            <div class="mb-4 p-4 bg-blue-50 border-l-4 border-blue-500 rounded">
                                ...
                                @if($query->resolved_at)
                                    <p class="text-xs text-blue-600 mt-2">Responded on {{ $query->resolved_at->format('M d, Y h:i A') }}</p>
                                @endif
                            </div>
                        @endif

                        <div class="border-t pt-4">
                            <button onclick="toggleResponseForm({{ $query->id }})" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
                        ...
                        <div id="response-form-{{ $query->id }}" class="hidden mt-4 border-t pt-4">
                            <form action="{{ route('admin.queries.respond', $query) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    ...
                                    <textarea name="admin_response" rows="4" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" placeholder="Type your response here...">{{ $query->admin_response }}</textarea>
                                </div>
                                <div class="flex items-center gap-3">
                                    <button type="submit" class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition">
                                        Send Response
                                    </button>
                                    <button type="button" onclick="toggleResponseForm({{ $query->id }})" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition">
                                        Cancel
                                    </button>
                                </div>
                            </form>
