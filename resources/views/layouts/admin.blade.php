        ...
        <aside class="bg-gradient-to-b from-surface-main to-surface-alt border-r border-ui-border shadow-lg w-full md:w-64 md:flex-shrink-0 overflow-y-auto hidden md:flex md:flex-col" id="adminSidebar">
            <div class="p-6 flex-1">
                ...
                @php
                    ...
                    $isNonAdmin  = !$sidebarUser->is_admin;
                ...
                <nav class="space-y-2">
                    ...
                    @if($isNonAdmin)
                    @php
                        $myProfileUrl = match($sidebarUser->role) {
                            'manager'   => route('admin.managers.show', $sidebarUser),
                            'rm'        => route('admin.rms.show', $sidebarUser),
                            'affiliate' => route('admin.affiliates.show', $sidebarUser),
                            default     => route('admin.profile.edit'),
                        };
                        $myProfileActive = match($sidebarUser->role) {
                            'manager'   => request()->routeIs('admin.managers.show') && request()->route('manager')?->id == $sidebarUser->id,
                            'rm'        => request()->routeIs('admin.rms.show') && request()->route('rm')?->id == $sidebarUser->id,
                            'affiliate' => request()->routeIs('admin.affiliates.show') && request()->route('user')?->id == $sidebarUser->id,
                            default     => request()->routeIs('admin.profile.*'),
                        };
                    @endphp
                    <a href="{{ $myProfileUrl }}" class="flex items-center px-4 py-3 text-text-heading rounded-lg hover:bg-gradient-to-r hover:from-brand-gold hover:via-brand-amber hover:to-brand-crimson hover:text-white transition-all duration-300 {{ $myProfileActive ? 'bg-gradient-to-r from-brand-gold via-brand-amber to-brand-crimson text-white shadow-md' : '' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    @endif {{-- end !$isNonAdmin admin-only block --}}
                </nav>
            </div>
            <div class="p-6 border-t border-gray-100 flex-shrink-0">
                @if(!$isNonAdmin && $canSee('users'))
