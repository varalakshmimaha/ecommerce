...
<div class="admin-card max-w-4xl">
    ...
    <form action="<?php echo e(route('admin.products.update', $product)); ?>" method="POST" enctype="multipart/form-data" class="space-y-6" id="product-form">
        ...
        <div class="border-t pt-6">
            ...
            <input type="hidden" name="related_products" id="related_products" value="<?php echo e($product->relatedProducts->pluck('id')->implode(',')); ?>">
            ...
            <div id="related-products-container" class="space-y-3 mb-6">
                <?php $__currentLoopData = $product->relatedProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $relatedProduct): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300 related-product-item" data-id="<?php echo e($relatedProduct->id); ?>">
                        <div class="flex items-center space-x-4">
                            <div class="relative">
                                <img src="<?php echo e(asset('storage/' . $relatedProduct->main_image)); ?>" alt="<?php echo e($relatedProduct->name); ?>" class="w-14 h-14 object-cover rounded-lg border border-gray-300">
                            ...
                            <div>
                                <p class="font-semibold text-gray-900"><?php echo e($relatedProduct->name); ?></p>
                                <div class="flex items-center space-x-3 mt-1">
                                    <span class="text-sm text-gray-600 bg-gray-100 px-2 py-0.5 rounded">SKU: <?php echo e($relatedProduct->sku); ?></span>
                                    <span class="text-xs text-[#D4AF37] bg-[#D4AF37]/10 px-2 py-0.5 rounded">₹<?php echo e(number_format($relatedProduct->selling_price, 2)); ?></span>
                                </div>
                            </div>
                        </div>
                        <button type="button" onclick="removeRelatedProduct(<?php echo e($relatedProduct->id); ?>)" class="text-gray-400 hover:text-red-500 transition-colors p-2 rounded-full hover:bg-red-50">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <?php if($product->relatedProducts->count() == 0): ?>
...
<script>
    ...
    let selectedProducts = new Set(<?php echo json_encode($product->relatedProducts->pluck('id')->toArray(), 15, 512) ?>);
    ...
    async function searchSuveeProducts(query) {
        ...
        try {
            const response = await fetch(`<?php echo e(route('admin.products.search')); ?>?search=${encodeURIComponent(query)}&exclude=<?php echo e($product->id); ?>`);
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/suveeindiaweb/htdocs/suveeindia.com/suvee/resources/views/admin/products/edit.blade.php ENDPATH**/ ?>